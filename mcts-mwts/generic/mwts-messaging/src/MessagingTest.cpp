...
void MessagingTest::OnInitialize()
{
    ...
    mEmailMasterName = readFromConf( "email-master-account-name" );
    mEmailSlaveAddress = readFromConf( "email-slave-account-address" );
...
void MessagingTest::SendEmail( const QString& subject, const QString& message )
{
    ...
    qDebug() << "MessagingTest::SendEmail Subject: " << subject;
    qDebug() << "MessagingTest::SendEmail Message: " << message;

	QMessageAccountIdList ids =
            mMsgManager->queryAccounts( QMessageAccountFilter::byName( mEmailMasterName ) );

	if ( ids.size() <= 0 )
	{
        qDebug() << "MessagingTest::SendEmail No accounts found with name " << mEmailMasterName;
        qCritical() << "MessagingTest::SendEmail No accounts found with name " << mEmailMasterName;
    ...
    qDebug() << "MessagingTest::SendEmail ids.takeFirst()";
    QMessageAccountId id = ids.takeFirst();

    qDebug() << "MessagingTest::SendEmail Account ID: " << id.toString();
	QMessageAccount account( id );
    qDebug() << "MessagingTest::SendEmail Account name: " << account.name();

    qDebug() << "MessagingTest::SendEmail account.messageTypes()";
    QMessage::TypeFlags msgTypes = account.messageTypes();
	if ( msgTypes & QMessage::Mms )
	{
        qDebug() << "MessagingTest::SendEmail Account supports MMS.";
	}
	if ( msgTypes & QMessage::Sms )
	{
        qDebug() << "MessagingTest::SendEmail Account supports SMS.";
	}
	if ( msgTypes & QMessage::Email )
	{
        qDebug() << "MessagingTest::SendEmail Account supports E-Mail.";
	}
    else
    {
        qDebug() << "MessagingTest::SendEmail Account DOES NOT supports E-Mail.";

        qCritical() << "MessagingTest::SendEmail Account DOES NOT supports E-Mail.";
        g_pResult->StepPassed( "SendEmail", false );
        mRetValue = false;
        MWTS_LEAVE;
        return;
	...
	if ( msgTypes & QMessage::InstantMessage )
	{
        qDebug() << "MessagingTest::SendEmail Account supports Instant Messages.";
	}

    QMessageAddress address( QMessageAddress::Email, mEmailSlaveAddress );
	QMessage msg;
    ...
    qDebug() << "MessagingTest::SendEmail sendMessage";
    bool ret = sendMessage( msg );
...
bool MessagingTest::sendMessage( QMessage& message )
{
	...
	if ( ret == true )
	{
        qDebug() << "MessagingTest::sendMessage Succesfully sent message!";
	}
	else
	{
        qCritical() << "Failed to send message!";
    ...
    mRetValue = ret;
    g_pResult->StepPassed( "sendMessage", mRetValue );
...
void MessagingTest::verifyMessage()
{
	...
	if ( msg.standardFolder() == QMessage::InboxFolder )
	{
		...
		switch ( msg.type() )
		{
			...
			case QMessage::InstantMessage:
				...
				break;
            case QMessage::AnyType:
                qDebug() << "The message is an anytype message object";
                break;
        }
    ...
    {
        qCritical() << "msg.standardFolder() != QMessage::InboxFolder! Something wrong with account";
        mRetValue = false;
    }
