...
int main (int argc, char ** argv){
...
#endif
	if (cpu_p_sys_write (1, "scaling_governor", "powersave") != SUCCESS){
		return ERROR;
	}

	if (cpu_p_sys_check (1, "scaling_governor", "powersave") != SUCCESS) {
		return SUCCESS;
	} else {
		printf ("set governor to powersave successfully! \n");
	}
	sleep(10);
	if (cpu_p_sys_check (1, "scaling_cur_freq", min_freq) != SUCCESS){
		return ERROR;
	} else {
		printf ("powersave governor has taken effective! The frequency is %s now \n", min_freq);
	}
	rc = SUCCESS;
	pthread_t thread;
	...
	for(i=0;i<5;i++){
		sleep(2);
		if (cpu_p_sys_check (1, "scaling_cur_freq", min_freq) == SUCCESS) {
	        printf ("frequency keeps minumal when there is workload\n");
		}
		else{
