...
static void
test_geoclue_master_client_create_position()
{
	GeoclueMasterClient* client = create_geoclue_master_client();
	g_assert ( client != NULL );

	GError* error = NULL;
	GeocluePosition* pos = geoclue_master_client_create_position ( client,
		&error );
	
	/*assert no error occurs*/
	if ( error ) {
		g_test_message ( "Error in geoclue_master_client_create_position:\
			%s.\n", error->message );
		g_assert_not_reached();
	}

	g_assert ( pos != NULL );
	g_assert ( GEOCLUE_IS_POSITION ( pos ) );
	...
	if ( error ) {
		g_test_message ( "Error in geoclue_position_get_position: %s.\n",
			error->message );
		g_assert_not_reached();
	}
...
int main ( int argc,char **argv )
{
	...
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
		test_geoclue_master_client_create_position",
		test_geoclue_master_client_create_position );
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
	...
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
		test_get_position_fields_latitude",test_get_position_fields_latitude );
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
	...
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
		test_get_position_fields_altitude",test_get_position_fields_altitude );
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
		test_get_position_accuracy_level",test_get_position_accuracy_level );
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
	...
	g_test_add_func (
		"/geoclue/geoclue_master_client/create_position/\
		test_get_position_accuracy_vertical",test_get_position_accuracy_vertical
		);
