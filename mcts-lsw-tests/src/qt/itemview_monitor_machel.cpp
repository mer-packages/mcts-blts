	...
	foreach (SwClientItem *item, itemsAdded)
		{
			...
			this->thisMap[item->getSwUUID()] = item;
		}
	this->machel();
}
	...
	foreach (SwItemId itemID, itemsRemoved)
		{
			...
			this->thisMap.remove(itemID.uuid);
		}
	this->machel();
}

void ItemViewMonitor::machel() {
	QList<SwClientItem*> items = this->thisMap.values();
	QString str_buf;
	foreach (SwClientItem * item, items)
		{
			str_buf.append(QString("[%1]\n").arg(item->getSwUUID()));
			QHash<QString, QString> propsHash = item->getSwItemProps();
			QHash<QString, QString>::iterator i;
			for (i = propsHash.begin(); i != propsHash.end(); ++i) {
				str_buf.append(QString("%1=%2\n").arg(i.key(), i.value()));
			}
			str_buf.append(QString("\n\n"));
		}

	QFile f(CACHE_FILE_NAME);
