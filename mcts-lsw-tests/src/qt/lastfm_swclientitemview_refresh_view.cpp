...
int main(int argc, char *argv[]) {
	...
	qDebug()<< "Opening and Starting a item view.";
	sleep(1);

	qDebug()<< "Going to invoke the method 'refreshView' to refresh the item view.";
	monitor->refreshMonitorView();

	qDebug()<< "Done, the item view is refreshed!";
	return a.exec();
}
