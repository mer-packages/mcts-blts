...
if __name__ == '__main__':
    ...
    if(case.need_record_caps_sig()):
        ...
        subp = subprocess.Popen(['./dbus_user_changed_signal.py', '-s', 'lastfm', '-c'])
    else:
        #Invoke the LSW thread to monitor DBus
        subp = subprocess.Popen(['./dbus_user_changed_signal.py', '-s', 'lastfm'])
    
    time.sleep(10)
    
    def getUserNameFromGnomeKeyRing():
        useraccount = ""
        f = os.popen("./reading_user_account")
    ...
    #ori_password = getInfoFromConftool("password")
    
    new_user = "moblintest2"
    new_password = "otc111"
    
    os.system("./setting_user_account -u %s -p %s" % (new_user, new_password))
    ...
    os.system("./setting_user_account -u %s -p %s" % (ori_user, new_password))
    incokeCredentialsUpdated()
