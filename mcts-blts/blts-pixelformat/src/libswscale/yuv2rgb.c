...
// This is exactly the same code as yuv2rgb_c_16 except that the endianness is swapped
YUV2RGBFUNC(yuv2rgb_c_16_swap, uint16_t)
	LOADCHROMA(0);
	PUTRGBSWAP(dst_1,py_1,0,0);
	PUTRGBSWAP(dst_2,py_2,0,1);

	LOADCHROMA(1);
	PUTRGBSWAP(dst_2,py_2,1,1);
	PUTRGBSWAP(dst_1,py_1,1,0);

	LOADCHROMA(2);
	PUTRGBSWAP(dst_1,py_1,2,0);
	PUTRGBSWAP(dst_2,py_2,2,1);

	LOADCHROMA(3);
	PUTRGBSWAP(dst_2,py_2,3,1);
	PUTRGBSWAP(dst_1,py_1,3,0);
CLOSEYUV2RGBFUNC(8)
...
SwsFunc sws_yuv2rgb_get_func_ptr(SwsContext *c)
{
    ...
    case PIX_FMT_RGB555BE:   return yuv2rgb_c_16_swap;
    case PIX_FMT_RGB8:
                                      ...
                                      int brightness, int contrast, int saturation)
{
    ...
    case 24:
        ...
        fill_table(c->table_rV, 1, crv, (uint8_t*)(y_table + yoffs));
        fill_table(c->table_gU, 1, cgu, (uint8_t*)(y_table + yoffs));
        fill_table(c->table_bU, 1, cbu, (uint8_t*)(y_table + yoffs));
        fill_gv_table(c->table_gV, 1, cgv);
        break;
    case 32:
    ...
    default:
        c->yuvTable = NULL;
        BLTS_ERROR("%ibpp not supported by yuv2rgb\n", bpp);
        return -1;
    }
    return 0;
}
