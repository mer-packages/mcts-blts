...
static gboolean
call_barring_timeout(gpointer data);
static void
setup_signal_callbacks(struct call_barring_state* state);
static void
remove_signal_callbacks(struct call_barring_state* state);

static void
on_call_barring_property_changed(DBusGProxy *proxy, char *key, GValue* value,
    gpointer user_data);
static void
on_call_barring_incoming_barring_in_effect(DBusGProxy *proxy,
    gpointer user_data);
static void
on_call_barring_outgoing_barring_in_effect(DBusGProxy *proxy,
    gpointer user_data);
...
ofono_call_barring_change_password(void* user_ptr, char* modem_path)
{
	...
	// change the password
	if (!org_ofono_CallBarring_change_password(proxy, data->old_pin,
	    data->new_pin, &error))
	{
		display_dbus_glib_error(error);
		g_error_free(error);
	...
	// give a small amount of time for oFono to pass messages
	sleep(1);
	// and change the password back to original
	if (!org_ofono_CallBarring_change_password(proxy, data->new_pin,
	    data->old_pin, &error))
	{
		display_dbus_glib_error(error);
		g_error_free(error);
...
int
ofono_call_barring_test(void* user_ptr, __attribute__((unused)) int test_num)
{
	my_ofono_data* data = (my_ofono_data*) user_ptr;
	int retval = 0;
	int i;
...
setup_signal_callbacks(struct call_barring_state* state)
{
	state->signalcb_CallBarring_PropertyChanged = G_CALLBACK(
	    on_call_barring_property_changed);
	...
	state->signalcb_CallBarring_IncomingBarringInEffect = G_CALLBACK(
	    on_call_barring_incoming_barring_in_effect);
	...
	state->signalcb_CallBarring_OutgoingBarringInEffect = G_CALLBACK(
	    on_call_barring_outgoing_barring_in_effect);
...
/* Timeout callback */
static gboolean
call_barring_timeout(gpointer user_data)
{
...
/* Signal callbacks */
static void
on_call_barring_property_changed(__attribute__((unused))DBusGProxy *proxy, char *key,
...
static void
on_call_barring_incoming_barring_in_effect(__attribute__((unused))DBusGProxy * proxy,
...
static void
on_call_barring_outgoing_barring_in_effect(__attribute__((unused))DBusGProxy * proxy,
    __attribute__((unused))gpointer user_data)
{
	/* NOTE: This signal was never received while testing. Either oFono does
	 not send it yet, or we have wrong expectations about its delivery. */
...
barring_variant_set_arg_processor(struct boxed_value *args, void *user_ptr)
{
	...
	data->old_pin = strdup(blts_config_boxed_value_get_string(args));
